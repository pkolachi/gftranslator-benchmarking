...
def parseNames(lin_idx, sentence, start):
    ...
    print "source language is %s and name string at %d is %s" %(SRCLANG, start, sentence[start:]);
	...
	print str(expr);
	return (expr, 0, end);
    print "finished parseNames";
    return None;

def parseUnknown(lin_idx, sentence, start):
    global GRAMMAR, SRCLANG;
    print "source language is %s and unknown string at %d is %s" %(SRCLANG, start, sentence[start:]);
    moving_start, end, eot = start, len(sentence), True;
